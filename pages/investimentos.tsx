...
const Investimentos: NextPage = () => {
    ...
    const Action = ({item, className}: any) => (<section className={className}>
            <a className="cursor-pointer" onClick={() => router.push(`cadastrar/${item._id}`)} > 
                <PencilIcon width="24" />
            </a>
            
            <a className="cursor-pointer" onClick={() => handleDelete(item._id || '')}> 
                <TrashIcon width="24"/> 
            </a>
        </section>
    )
    ...
    return <div>
        ...
        {userPurchase ? (<table className="hidden md:block w-11/12">
            <thead>
            ...
            <tbody className="border">
                {userPurchase && userPurchase.map((item, index) => (<tr className="border-b text-center" key={index}>
                        ...
                        <td >
                            <Action className="flex items-center justify-center gap-4" item={item}/>
                        </td>
                    </tr>)
                )}
            </tbody>
        ...
        {userPurchase?.map((item, index) => <ItemCard bitcoin={item} key={item._id} />)}
